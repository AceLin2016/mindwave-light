...
// void cleanProtocol(uint8_t array[], uint16_t _length ){ /* 本函数不完整，只能够正常清除char类型指针 */
//      //memset(array, 0, sizeof(char)*length);
// 	 uint16_t i = 0;
// 	 for( i=0; i<_length; i++)	array[i]=0;
// } 
...
//     if( (_mwLight->N_index==0)&&(_mwLight->protocolData[0]== SYN) ) { _mwLight->N_index++; SYN_ON(); }
...
//      else if( (_mwLight->N_index>6) && (_mwLight->N_index<35) ) { _mwLight->N_index++; SYN_OFF(); }
